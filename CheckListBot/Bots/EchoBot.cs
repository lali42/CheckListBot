// Copyright (c) Microsoft Corporation. All rights reserved.
// Licensed under the MIT License.
//
// Generated with Bot Builder V4 SDK Template for Visual Studio EchoBot v4.14.0

using Microsoft.Bot.Builder;
using Microsoft.Bot.Schema;
using System.Collections.Generic;
using System.Linq;
using System.Threading;
using System.Threading.Tasks;

namespace CheckListBot.Bots
{
    public class EchoBot : ActivityHandler
    {
        private static readonly MemoryStorage _myStorage = new MemoryStorage();

        private readonly IStatePropertyAccessor<CheckList> _checkListAccessor;
        public List<string> UtteranceList { get; set; } = new List<string>();
        public int TurnNumber { get; set; }

        public EchoBot(UserState userState)
        {
            _checkListAccessor = userState.CreateProperty<CheckList>("CheckList");
        }

        protected override async Task OnMessageActivityAsync(ITurnContext<IMessageActivity> turnContext, CancellationToken cancellationToken)
        {
            var utterance = turnContext.Activity.Text;
            //var listItems = UtteranceList;
            UtteranceList.Add(utterance);
            TurnNumber = UtteranceList.Count();

            if (TurnNumber == 0)
            {
                var replyText = $"Don't have item.";
                await turnContext.SendActivityAsync(MessageFactory.Text(replyText, replyText), cancellationToken);
            }
            else
            {
                for (int i = 0; i < UtteranceList.Count(); i++)
                {
                    var replyText = $"{TurnNumber} : {UtteranceList[i]}";
                    await turnContext.SendActivityAsync(MessageFactory.Text(replyText, replyText), cancellationToken);
                }
            }
        }

        protected override async Task OnMembersAddedAsync(IList<ChannelAccount> membersAdded, ITurnContext<IConversationUpdateActivity> turnContext, CancellationToken cancellationToken)
        {
            var welcomeText = "Hello and welcome!";
            foreach (var member in membersAdded)
            {
                if (member.Id != turnContext.Activity.Recipient.Id)
                {
                    await turnContext.SendActivityAsync(MessageFactory.Text(welcomeText, welcomeText), cancellationToken);
                }
            }
        }
    }
}
